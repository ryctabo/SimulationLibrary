 ...
 * @version 1.0
 */
public class KolmogorovTable {

    private static final float alpha[] = {
        0.2f, 0.1f, 0.05f, 0.02f, 0.01f, 0.005f, 0.002f, 0.001f 
    };
    ...
    public static final double[][] VALUES = {
        ...
        {0.21646, 0.24746, 0.27490, 0.30728, 0.32954, 0.35027, 0.37575, 0.39380},
    ...
    private static KolmogorovTable singleton;

    private KolmogorovTable() {
        this.alphaList = new ArrayList<>();
        
        for (float value : alpha) {
            this.alphaList.add(value);
        }
    }
    
    public static KolmogorovTable getInstance() {
        if (singleton == null)
            singleton = new KolmogorovTable();
        
        return singleton;
    }
    
    public double getValue(float alpha, int n) {
        if (n < 0) {
            throw new TestException("El número de muestras dadas (N) debe ser mayor que 0.");
        }
        
        if (n > 50) {
            if (alphaList.contains(alpha)) {
                int position = alphaList.lastIndexOf(alpha);
                switch (position) {
                    case 0: return 1.07d / Math.sqrt(n);
                    case 1: return 1.22d / Math.sqrt(n);
                    case 2: return 1.36d / Math.sqrt(n);
                    case 3: return 1.52d / Math.sqrt(n);
                    case 4: return 1.63d / Math.sqrt(n);
                    case 5: return 1.73d / Math.sqrt(n);
                    case 6: return 1.85d / Math.sqrt(n);
                    case 7: return 1.95d / Math.sqrt(n);
                    default:
                        throw new AssertionError();
                }
            } else {
                throw new UnidentifiedAlphaException("Alpha no identificado, no "
                        + "se encuentra entre las significancias que tenemos en cuenta.\n"
                        + "Los grados de significancia que tenemos en cuenta son:"
                        + "0.2f, 0.1f, 0.05f, 0.02f, 0.01f, 0.005f, 0.002f, 0.001f.");
            }
        } else {
            if (!alphaList.contains(alpha)) {
