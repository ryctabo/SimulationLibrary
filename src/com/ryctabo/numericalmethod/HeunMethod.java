 ...
 * @version 1.0
 */
public class HeunMethod implements NumericalMethodXY {
    ...
    public HeunMethod(double limit_inf, double limit_sup, double initialCondition, double stepSize) {
    ...
    public HeunMethod(double limit_inf, double limit_sup, double initialCondition, int iterations) {
        this.limit_inf = limit_inf;
        this.limit_sup = limit_sup;
        this.initialCondition = initialCondition;
    ...
    public void generateSolution(EquationXY eq) {
        ...
        for (double i = this.limit_inf + this.stepSize; i <= this.limit_sup; i += this.stepSize) {
            ...
            double sol = values[nIterations] + (k1 + 3 * k3) / 4;
    ...
    private void calculateK2(double x, double y, EquationXY eq) {
        this.k2 = stepSize * eq.equation(x + k1 / 3, y + stepSize / 3);
    }

    private void calculateK3(double x, double y, EquationXY eq) {
        this.k3 = stepSize * eq.equation(x + 2 * k2 / 3, y + 2 * stepSize / 3);
    }
